...
### When should I use CHTC?

| Factor | Laptop | CHTC |
|--------|--------|------|
| **Cost** | Hardware purchase | Free to UW-Madison researchers |
| **GPUs** | Consumer GPU (if any) | A100 40/80 GB, H100 80 GB, H200 141 GB |
| **Parallelism** | 1 job at a time | Hundreds of jobs in parallel |
| **Job time limits** | None | 12 hrs (short), 24 hrs (medium), 7 days (long) |
| **Queue wait times** | None | Minutes to hours (varies by demand) |
| **Setup** | Install packages locally | SSH + submit files + containers |
| **Internet** | Full | Workers typically have restricted internet |

**Use CHTC when you need:**
- GPUs you don't have locally
- To run many parallel experiments (hyperparameter sweeps, cross-validation)
- Hardware beyond what your laptop offers (memory, disk, compute)
- Reproducible containerized environments

**Your laptop may be fine when:**
- The model is small and trains in minutes
- You're doing exploratory data analysis
- You need constant interactive feedback (plotting, debugging)

Many researchers develop and test locally, then move to CHTC for production training and experimentation.

### What hardware does CHTC have?
...
| Frontier | 70B+ | GPT-4-scale, multi-expert models | Multi-node clusters (beyond CHTC's current scope) |

**CHTC's [GPU Lab](https://chtc.cs.wisc.edu/uw-research-computing/gpu-lab) covers more than you might think.** A100s, H100s, and H200s provide enough VRAM to run inference or fine-tune models up to ~70B parameters with quantization.
...
**Containerized environments.** CHTC uses **Apptainer** (formerly Singularity) containers to provide reproducible software environments. You can pull containers from Docker Hub or build your own.

## The submit-node-as-controller pattern

The central idea of this workshop: you work on a lightweight **submit node** — a shared login machine accessible via SSH — and use **HTCondor** to dispatch compute-heavy jobs to worker machines in the pool. The submit node itself does not run heavy compute. Instead, it orchestrates:
...
On CHTC, **the command line is the primary interface** — there is no web console for job management. Everything is done via SSH and terminal commands. If you're more comfortable with graphical interfaces, don't worry — the commands are straightforward and this workshop walks you through each one. You can also use VS Code's Remote-SSH extension for a more visual editing experience while still using the terminal for HTCondor commands.
...
- What hardware do you use — laptop, HPC cluster, something else?
- What's the biggest infrastructure pain point in your workflow (GPU access, environment setup, data transfer)?
