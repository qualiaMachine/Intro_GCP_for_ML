...
- The container image switches to the GPU-enabled version (`pytorch-xla.2-4.py310:latest`), which includes CUDA and cuDNN.
...
3. **Container choice**: We used `pytorch-xla.2-4.py310` for both the CPU and GPU jobs. What would happen if you used a CPU-only container but still passed `accelerator_type` and `accelerator_count`?
